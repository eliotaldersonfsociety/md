...
import { Gift, Sparkles, Truck, MessageCircle, Star, ArrowRight } from "lucide-react"
import { allProducts } from "@/lib/products-data"
import { useState } from "react"
import { buildProductSlug } from "@/lib/slugify"
import { WhatsAppModal } from "@/components/whatsapp-modal"

const navidadProducts = allProducts
...
export default function NavidadClient() {
  ...
  const openWhatsApp = (productName: string) => {
    setWhatsAppMessage(
      encodeURIComponent(
        `Hola, me interesa el producto "${productName}" para Navidad. ¿Tienen disponibilidad y cuál es el tiempo de entrega?`
      )
    )
    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="relative min-h-[60vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-4 py-20 max-w-4xl mx-auto">
          <span className="inline-block px-4 py-1.5 rounded-full bg-white/20 text-white text-sm font-medium mb-6 backdrop-blur-sm">✨ 25 de Diciembre</span>
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold text-white mb-6 text-balance leading-tight">
            Peluches y Regalos para <span className="text-pink-300">Navidad</span>
          </h1>
          <p className="text-lg md:text-xl text-white/90 mb-8 max-w-2xl mx-auto leading-relaxed">Los peluches más tiernos y regalos únicos para esta temporada navideña. Envíos a Colombia y Venezuela.</p>
      ...
      <section id="productos" className="py-20 bg-background">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4">Regalos Navideños</h2>
            <p className="text-muted-foreground max-w-2xl mx-auto text-lg">Los peluches más tiernos para regalar en Navidad.</p>
          </div>
            ...
            {navidadProducts.map((product) => {
              ...
              return (
                <div key={product.id} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border">
                  ...
                  <div className="p-4">
                    <span className="text-xs text-primary font-medium uppercase tracking-wide">{product.category === "peluches" ? "Peluche" : "Arreglo"}</span>
      ...
      <section className="py-20 bg-muted/30">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4">Ideas de Regalos para Navidad</h2>
            <p className="text-muted-foreground max-w-2xl mx-auto text-lg">Inspírate con nuestras ideas navideñas.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-8 mb-12">
            <div className="bg-white rounded-2xl p-8 shadow-sm border border-border hover:border-primary/20 transition-colors">
              ...
              <h3 className="text-xl font-bold text-foreground mb-3">Peluches Navideños</h3>
              <p className="text-muted-foreground mb-4">Peluches personalizados con motivos navideños. Un regalo único para esta temporada.</p>
            ...
            <div className="bg-white rounded-2xl p-8 shadow-sm border border-border hover:border-primary/20 transition-colors">
              ...
              <p className="text-muted-foreground mb-4">Combina peluches con detalles navideños para crear el regalo perfecto.</p>
            ...
            <div className="bg-white rounded-2xl p-8 shadow-sm border border-border hover:border-primary/20 transition-colors">
              <div className="w-14 h-14 rounded-full bg-primary/10 flex items-center justify-center mb-6"><Truck className="h-7 w-7 text-primary" /></div>
              <h3 className="text-xl font-bold text-foreground mb-3">Envíos a Tiempo</h3>
              <p className="text-muted-foreground mb-4">Garantizamos entregas antes de Navidad. Haz tu pedido con anticipación.</p>
              <Link href="/envios" className="text-primary font-semibold hover:underline inline-flex items-center gap-1">Ver envíos <ArrowRight className="h-4 w-4" /></Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 bg-gradient-to-br from-primary to-primary/80">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">¿Listo para celebrar Navidad?</h2>
          <p className="text-white/90 text-lg mb-8 max-w-2xl mx-auto">Contáctanos ahora y asegura tu regalo navideño a tiempo.</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Button size="lg" variant="outline" className="rounded-full px-8 border-white/30 text-white hover:bg-white/10 font-semibold" onClick={() => openWhatsApp("Cotización Navidad")}><MessageCircle className="mr-2 h-5 w-5" /> WhatsApp</Button>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 bg-background">
        <div className="container mx-auto px-4">
          ...
          <div className="max-w-3xl mx-auto space-y-4">
            {[
              { question: "¿Cuándo debo hacer mi pedido para Navidad?", answer: "Te recomendamos hacer tu pedido con al menos 7 días hábiles de anticipación para garantizar la entrega antes del 25 de diciembre." },
              { question: "¿Puedo personalizar mi regalo navideño?", answer: "Sí, todos nuestros productos se pueden personalizar con nombres, mensajes o diseños navideños especiales." },
              { question: "¿Qué regalo es más popular para Navidad?", answer: "Los peluches personalizados y los arreglos con detalles navideños son los más elegidos para esta fecha." },
              { question: "¿Hacen envíos en fechas navideñas?", answer: "Sí, realizamos envíos especiales en diciembre. Consulta nuestras fechas límite para garantizar la entrega." },
              { question: "¿Hay descuentos por compras al por mayor en Navidad?", answer: "Sí, tenemos descuentos especiales para pedidos empresariales. Contáctanos para más información." }
            ].map((faq, index) => (
      ...
      <section className="py-16 bg-muted/30">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
            <Link href="/peluches" className="bg-white rounded-xl p-6 text-center border border-border hover:border-primary/20 transition-colors"><Gift className="h-8 w-8 text-primary mx-auto mb-3" /><h3 className="font-semibold text-foreground">Peluches</h3><p className="text-sm text-muted-foreground">Ver todos</p></Link>
            <Link href="/cojines" className="bg-white rounded-xl p-6 text-center border border-border hover:border-primary/20 transition-colors"><Sparkles className="h-8 w-8 text-primary mx-auto mb-3" /><h3 className="font-semibold text-foreground">Cojines</h3><p className="text-sm text-muted-foreground">Ver todos</p></Link>
