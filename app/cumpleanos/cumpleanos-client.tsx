...
import { Cake, Heart, Sparkles, Truck, MessageCircle, Star, ArrowRight } from "lucide-react"
import { allProducts } from "@/lib/products-data"
import { useState } from "react"
import { buildProductSlug } from "@/lib/slugify"
import { WhatsAppModal } from "@/components/whatsapp-modal"

const cumpleProducts = allProducts
  .filter(p => p.category === "peluches" || p.category === "floristeria")
  .slice(0, 8)

export default function CumpleanosClient() {
  ...
  const openWhatsApp = (productName: string) => {
    setWhatsAppMessage(
      encodeURIComponent(
        `Hola, me interesa el producto "${productName}" para un cumpleaños. ¿Tienen disponibilidad y cuál es el tiempo de entrega?`
      )
    )
    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="relative min-h-[60vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-4 py-20 max-w-4xl mx-auto">
          <span className="inline-block px-4 py-1.5 rounded-full bg-white/20 text-white text-sm font-medium mb-6 backdrop-blur-sm">✨ Celebra su día</span>
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold text-white mb-6 text-balance leading-tight">
            Regalos para <span className="text-pink-300">Cumpleaños</span>
          </h1>
          <p className="text-lg md:text-xl text-white/90 mb-8 max-w-2xl mx-auto leading-relaxed">Peluches personalizados y regalos únicos para cumpleaños. Sorprende a esa persona especial. Envíos a Colombia y Venezuela.</p>
      ...
      <section id="productos" className="py-20 bg-background">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4">Regalos para Cumpleaños</h2>
            <p className="text-muted-foreground max-w-2xl mx-auto text-lg">Los peluches y arreglos más queridos para regalar en cumpleaños.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {cumpleProducts.map((product) => {
      ...
      <section className="py-20 bg-muted/30">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <span className="inline-block px-4 py-1.5 rounded-full bg-primary/10 text-primary text-sm font-medium mb-4">Ideas por relación</span>
            <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4">Regalos para cada persona</h2>
            <p className="text-muted-foreground max-w-2xl mx-auto text-lg">Encuentra el regalo perfecto para cada cumpleaños.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-8 mb-12">
            <div className="bg-white rounded-2xl p-8 shadow-sm border border-border hover:border-primary/20 transition-colors text-center">
              ...
              <p className="text-muted-foreground mb-4">Peluches románticos y cojines con mensajes de amor para sorprender a tu pareja.</p>
            ...
            <div className="bg-white rounded-2xl p-8 shadow-sm border border-border hover:border-primary/20 transition-colors text-center">
              <div className="text-4xl mb-4">👫</div>
              <h3 className="text-xl font-bold text-foreground mb-3">Para amigos</h3>
              <p className="text-muted-foreground mb-4">Detalles divertidos y originales para celebrar a tus amigos.</p>
              <Button asChild className="w-full rounded-full bg-primary hover:bg-primary/90"><Link href="/peluches">Ver opciones <ArrowRight className="ml-2 h-4 w-4" /></Link></Button>
            </div>
            <div className="bg-white rounded-2xl p-8 shadow-sm border border-border hover:border-primary/20 transition-colors text-center">
              <div className="text-4xl mb-4">👨‍👩‍👧‍👦</div>
              <h3 className="text-xl font-bold text-foreground mb-3">Para familiares</h3>
              <p className="text-muted-foreground mb-4">Regalos cálidos y personalizados para padres, hermanos y seres queridos.</p>
              <Button asChild className="w-full rounded-full bg-primary hover:bg-primary/90"><Link href="/cojines">Ver cojines y detalles <ArrowRight className="ml-2 h-4 w-4" /></Link></Button>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 bg-gradient-to-br from-primary to-primary/80">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">¿Listo para celebrar un cumpleaños?</h2>
          <p className="text-white/90 text-lg mb-8 max-w-2xl mx-auto">Envíos a todo Colombia y Venezuela. Haz de este cumpleaños una fecha inolvidable.</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Button size="lg" variant="outline" className="rounded-full px-8 border-white/30 text-white hover:bg-white/10 font-semibold" onClick={() => openWhatsApp("Cotización Cumpleaños")}><MessageCircle className="mr-2 h-5 w-5" /> WhatsApp</Button>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 bg-background">
        <div className="container mx-auto px-4">
          ...
          <div className="max-w-3xl mx-auto space-y-4">
            {[
              { question: "¿Cuándo debo hacer mi pedido para un cumpleaños?", answer: "Te recomendamos hacer tu pedido con al menos 5 días hábiles de anticipación para garantizar la entrega a tiempo." },
              { question: "¿Puedo personalizar el regalo con el nombre del cumpleañero?", answer: "Sí, todos nuestros peluches y cojines se pueden personalizar con nombres, fechas o mensajes especiales." },
              { question: "¿Qué regalo es más popular para cumpleaños?", answer: "Los peluches personalizados y los cojines con frases especiales son los más elegidos para cumpleaños." },
      ...
      <section className="py-16 bg-muted/30">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
            <Link href="/peluches" className="bg-white rounded-xl p-6 text-center border border-border hover:border-primary/20 transition-colors"><Cake className="h-8 w-8 text-primary mx-auto mb-3" /><h3 className="font-semibold text-foreground">Peluches</h3><p className="text-sm text-muted-foreground">Ver todos</p></Link>
            <Link href="/cojines" className="bg-white rounded-xl p-6 text-center border border-border hover:border-primary/20 transition-colors"><Heart className="h-8 w-8 text-primary mx-auto mb-3" /><h3 className="font-semibold text-foreground">Cojines</h3><p className="text-sm text-muted-foreground">Ver todos</p></Link>
